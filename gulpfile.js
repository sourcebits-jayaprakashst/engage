...
gulp.task('jshint', function(){
    gulp.src(['!node_modules/**','!.grunt/**','!dist/**','!bower_components/**','**/*.js'])
        .pipe(jshint())
        .pipe(jshint.reporter(stylish)).pipe(connect.reload());
});
...
gulp.task('connect', function() {
  connect.server({
    ...
    gulp.src('')
    .pipe(opened({ uri: 'http://localhost:8080/#/state1'}));
});

gulp.task('updatebuild',['build'],function(){
        gulp.src('')
        .pipe(connect.reload());
        console.log('File was changed server browser restarted');
});

gulp.task('watch', function () {
    var watcher = gulp.watch('js/**/*.js', ['updatebuild']);
    watcher = gulp.watch('styles/**/*.less', ['updatebuild']);
    watcher = gulp.watch('partials/**/*.html', ['updatebuild']);
...
gulp.task('default', ['build', 'watch','connect'],function(){
    gulp.src('')
    .pipe(connect.reload());
    console.log('File was changed server browser started');
});
